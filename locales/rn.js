export default {
  // System
  'lang': 'rn',
  'lang-name': 'Ikirundi',

  // Global
  'UN OCHA': 'UN OCHA',
  'UN Office for the Coordination of Humanitarian Affairs': 'Ibiro mpuzamakungu vya ONU bijejwe gushira hamwe inguvu z\'amashirahamwe ajejwe gutabara mubihe hadutse ibiza bitandukanye',
  'Situation Report': 'Icegeranyo c\'ivyabaye',
  'Situation Reports': 'Icegeranyo c\'ivyabaye',

  // Errors
  'No data available.': 'Inkuru ntishoboye kuboneka',
  'Page not found': 'Iyo nkuru ntishoboye kuboneka',
  'Server Error': 'Ingorane y\'icuma nyabwonko kibika amakuru',
  'We want to help you find the information you are looking for.': 'Dushaka kubafasha kurondera inkuru mwifuza.',
  'Here are some of OCHA\'s latest Situation Reports to help you get back on track:': 'Ngaha murahasanga bimwe muvyegeranyo vya vuba vya OCHA:',
  'The URL you requested exists, but there were problems displaying the page.': 'Aho musanga inkuru mukeneye hariho, ariko kubera ingorane, ntibishobotse kohuguruka',

  // AppBar
  'Toggle menu': 'Ibirimwo',
  'Home': 'Intangamarara',
  'Latest updates': 'Amakuru mashasha',
  'About': 'Kubijanye',
  'OCHA Services': 'Ibikorwa vy\'ibiro mpuzamakungu vya ONU OCHA',

  // AppHeader
  'Last updated': 'Amakuru mashasha',
  'Subscribe': 'Iyandikishe',
  'Share': 'Sabikanya',
  'Email': 'Ubutumwa',
  'Download': 'Ugurura',
  'Archive': 'Amakuru yakera',
  'Read this Situation Report in a different language:': 'Soma ano makuru mundimi zitandukanye',

  // AppFooter
  'Privacy policy': 'Ikingirwa ry\'akabanga',
  'href-privacy': 'https://www.un.org/en/sections/about-website/privacy-notice/',
  'Copyright notice': 'Ikingirwa ry\'ibikorwa mvamutwe',
  'href-copyright': 'https://www.un.org/en/sections/about-website/copyright/index.html',
  'OCHA coordinates the global emergency response to save lives and protect people in humanitarian crises. We advocate for effective and principled humanitarian action by all, for all.': 'Ibiro vya OCHA bijejwe gutunganya inyishu zihuta mu ntumbero yo gutabara ubuzima bwa kiremwa muntu  mu gihe ashikiwe n\'ibiza bitandukanye. Tujejwe kandi guhimiriza kwishigwa mu ngiro, rya bose no kubwa bose, ry\'ibikorwa vy\'ubutabazi bushemeye kandi bushingiye ku mategeko ndanga butabazi.',

  // Homepage
  'About this site': 'About this site',
  'Recently updated': 'Amakuru yavuba',

  // Highlights
  'Highlights': 'Inkuru nyamukuru',

  // Key Figures
  'Key Figures': 'Ibiharuro nyamukuru',

  // Funding
  'Funding': 'Amafaranga',
  'Required': 'Amafaranga akenewe',
  'Received': 'Amafaranga amaze kuboneka',
  'Progress': 'Urugero rw\'amafaranga yabonetse',
  'Funding data could not be found.': 'Amakuru ajanye n\'amafaranga ntashoboye kuboneka',
  'No funding data available.': 'Amakuru ajanye n\'amafaranga ntayahari',

  // Contacts
  'Contacts': 'Mudukeneye',

  // Clusters
  'Cluster Status': 'Amakuru y\'igisata',
  'Sector Status': 'Amakuru y\'igisata',
  'Needs': 'Ibikenewe',
  'Response': 'Inyishu',
  'Gaps': 'Ibihengeri',

  // Articles
  'Access': 'Gushikira',
  'Analysis': 'Gusuzuma',
  'Background': 'Inkuru iramvuye',
  'Coordination': 'Guhuza ibikorwa',
  'Emergency Response': 'Gutabara vyihuta',
  'Feature': 'Inkuru nyamukuru',
  'Forecast': 'Ivyateguwe',
  'Media': 'Ibinyamakuru',
  'Trends': 'Intumbero',
  'Visuals and Data': 'Ibicapo n\'ibiharuro',
  'Read more': 'Soma inkuru yose',
  'Read less': 'Soma igice',

  // Flash Updates
  'Flash Update': 'Inkuru y\'incamake',

  // Videos
  'Video': 'Ireresi',
  'View this Video': 'Raba iyi reresi',

  // Interactives
  'Interactive': 'Ihindagurika',
  'View this interactive graphic': 'Raba iki igicapo gihindagurika',

  // Visuals
  'Visual': 'Ibicapo',

  // Social media
  'See entire digital situation report for [COUNTRY]': 'Raba icegeranyo c\'amakuru yose  kubijanye n\'[COUNTRY]',
  'See entire digital situation report': 'Raba icegeranyo cose',
  'Read the latest from [COUNTRY]\'s Situation Report': 'Soma icegeranyo ca vuba c\'[COUNTRY]',

  // Card Actions
  'Copy [THING] URL to clipboard': 'Imura iyi [THING] URL mu kibanza',
  '[THING] URL copied': '[THING] URL yimuwe',
  'Save Situation Report as PDF': 'Bika kino cegeranyo muri PDF',
  'Save [THING] as PNG': 'Bika ibintu [THING] nkisanamu(PNG)',

  // Snap strings
  'Date of Creation': 'Igenekerezo yakoreweko',
  'Date': 'Igenekerezo',
  'Downloaded': 'Yuguruwe',
  'Page # of #': 'urupapuro # rwa #',

  // Timestamps
  '# minute ago': 'Haheze umunota',
  '# minutes ago': '# Iminota iheze',
  '# hour ago': 'Haheze isaha',
  '# hours ago': '# Amasaha aheze',
  '# day ago': 'Haheze umusi',
  '# days ago': '# Iminsi iheze',
  '# week ago': 'Haheze indwi',
  '# weeks ago': '# Indwi ziheze',
  '# month ago': 'Haheze ukwezi',
  '# months ago': '# Amezi aheze',
  '# year ago': 'Haheze umwaka',
  '# years ago': '# Imyaka iheze',

  // Languages
  'Arabic': 'Icarabu',
  'English': 'Icongereza',
  'French': 'Igifaransa',
  'Russian': 'Ikirusiya',
  'Somali': 'Igisomaliya',
  'Spanish': 'Ikiespanyole',
  'Ukrainian': 'Ikinyawukereniya',
  'Kirundi': 'Ikirundi',
}
