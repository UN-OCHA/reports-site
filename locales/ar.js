export default {
  // System
  'lang': 'ar',
  'lang-name': 'عربي',

  // Global
  'UN OCHA': 'أوتشا',
  'UN Office for the Coordination of Humanitarian Affairs': 'مكتب الأمم المتحدة لتنسيق الشؤون الإنسانية',
  'Situation Report': 'تقرير الوضع',
  'Situation Reports': 'تقارير الوضع',

  // Errors
  'No data available.': 'لا يوجد بيانات متوفرة',
  'Page not found': 'الصفحة غير موجودة',
  'We want to help you find the information you are looking for.': 'نريد مساعدتك في العثور على المعلومات التي تريد.',
  'Here are some of OCHA\'s latest Situation Reports to help you get back on track:': 'إليك أخر تقارير الحالة من أوتشا التي قد تساعدك',

  // AppBar
  'Toggle menu': 'قائمة التبديل',
  'Home': 'الرئيسية',
  'Latest updates': 'آخر المستجدات',
  'About': 'حول',
  'OCHA Services': 'خدمات أوتشا',

  // AppHeader
  'Last updated': 'أخر التحديثات',
  'Subscribe': 'إشتراك',
  'Share': 'مشاركة',
  'Email': 'بريد إلكتروني',
  'Download': 'تنزيل',
  'Archive': 'أرشيف',
  'Read this Situation Report in a different language:': 'إقراء تقرير الوضع بلغة أخرى',

  // AppFooter
  'Privacy policy': 'اتفاقية الخصوصية',
  'href-privacy': 'https://www.un.org/ar/sections/about-website/privacy-notice/',
  'Copyright notice': 'حقوق الملكية',
  'href-copyright': 'https://www.un.org/ar/sections/about-website/copyright/index.html',
  'OCHA coordinates the global emergency response to save lives and protect people in humanitarian crises. We advocate for effective and principled humanitarian action by all, for all.': 'مھمة مكتب تنسیق الشؤون الإنسانية یقوم مكتب تنسیق الشؤون الإنسانية بتنسيق الاستجابة العالمية لحالات الطوارئ لإنقاذ الأرواح وحمایة الناس في الأزمات الإنسانية. ونحن ندعو إلى العمل الإنساني الفعال والقائم على المبادئ من قبل الجميع لصالح الجميع.',

  // Homepage
  'About this site': 'حول هذا الموقع',
  'Recently updated': 'أخر التحديثات',
  'The Digital Situation Report aims to simplify OCHA\'s current portfolio of field reporting products (Flash Update, Situation Report and Humanitarian Bulletin) by moving out of static PDFs and consolidating into a single online format. It will be more dynamic, visual, and analytical. The platform will save users\' time by automating distribution and design.': 'يهدف تقرير الحالة الرقمية إلى تبسيط المجموعة الحالية من منتجات التقارير الميدانية التي يصدرها مكتب تنسيق الشؤون الإنسانية (المستجدات الإنسانية الطارئة وتقرير الحالة والنشرة الإنسانية) من خلال الانتقال من ملفات الـPDF الجامدة وتوحيدها بصيغة إلكترونية واحدة، لتصبح بذلك أكثر ديناميكية وبصرية وتحليلية. وستوفر المنصة وقت المستخدمين من خلال أتمتة التوزيع والتصميم.',
  'As the system develops further, it will be adapted to pull data and information automatically from other platforms, which will promote consistency across products and facilitate access to wider analysis. By moving to modular, online content, OCHA will advance significantly in its humanitarian reporting.': 'ومع تطور النظام، سيتم تكييفه لسحب البيانات والمعلومات تلقائياً من منصات أخرى، مما سيعزز الاتساق عبر المنتجات ويسهل الوصول إلى تحليل أوسع نطاقاً. ومن خلال الانتقال إلى المحتوى المرن على الإنترنت، سيحقق مكتب تنسيق الشؤون الإنسانية تقدماً كبيراً في كتابة التقارير الإنسانية.',

  // Key Messages
  'Key Messages': 'الرسائل الرئيسية',
  'Highlights': 'أبرز الأحداث',

  // Key Figures
  'Key Figures': 'الأرقام الرئيسية',

  // Funding
  'Funding': 'التمويل',
  'Requirements': 'الاحتياجات',
  'Progress': 'التطور',
  'Funding data could not be found.': 'بيانات التمويل غير متوفرة',

  // Contacts
  'Contacts': 'اتصال بنا',

  // Clusters
  'Cluster Status': 'وضع المجموعات',
  'Needs': 'الاحتياجات',
  'Response': 'استجابة',
  'Gaps': 'الفجوات',

  // Articles
  'Access': 'الوصول',
  'Analysis': 'تحليل',
  'Background': 'خلفية',
  'Coordination': 'تنسيق',
  'Emergency Response': 'الاستجابة للطوارئ',
  'Feature': 'مقال',
  'Forecast': 'التنبؤ',
  'Media': 'وسائط',
  'Trends': 'الاتجاهات',
  'Visuals and Data': 'المواد البصرية والبيانات',
  'Read more': 'إقراء المزيد',
  'Read less': 'إقراء أقل',

  // Flash Updates
  'Flash Update': 'موجز المستجدات',

  // Videos
  'Video': 'فيديو',

  // Interactives
  'Interactive': 'تفاعلي',

  // Social media
  'Read the latest from COUNTRY\'s Situation Report': 'Read the latest from COUNTRY\'s Situation Report',

  // Snap strings
  'Date of Creation': 'تاريخ الإنشاء',
  'Date': 'التاريخ',
  'Downloaded': 'تم التنزيل',
  'Page # of #': 'الصفحة # من #',

  // Timestamps
  '# minute ago': 'منذ دقيقة',
  '# minutes ago': 'منذ # دقائق',
  '# hour ago': 'منذ ساعة',
  '# hours ago': 'منذ # ساعة',
  '# day ago': 'منذ يوم',
  '# days ago': 'منذ # يوم',
}
